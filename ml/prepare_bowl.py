...
def classify_category(name: str) -> tuple[bool, str]:
    ...
    if normalized in VALID_BOWL_NAMES:
        return True, f"{name!r} is a clearly valid bowl category."

    if raw in AMBIGUOUS_BOWL_NAMES or normalized in AMBIGUOUS_BOWL_NAMES:
        return False, f"{name!r} is ambiguous or a different object type (plate/pot/cup/container), not bowl."

    return False, f"{name!r} is not clearly a bowl annotation label."
        ...
        for leaf in ("images", "labels"):
            ...
            for path in folder.iterdir():
                if path.is_file() and path.name != ".gitkeep":
                    path.unlink()
                    removed += 1
    print(f"Cleared {removed} existing bowl image/label file(s), preserving .gitkeep placeholders.")
    print()
...
def summarize_records(
    ...
    for record in records:
        ...
        for category in categories:
            ...
            if accepted and instance_count > 0:
                ...
                decision = "valid bowl label, but zero instances -> no labels copied (Roboflow root/supercategory placeholder)"
    ...
    if excluded_reasons:
        print("WARNING: Ambiguous or non-bowl classes were found and will be excluded:")
...
def update_sourcing_notes(
    ...
    new_row = f"| bowl | {status} | {source} | {notes} |"
...
def main() -> None:
    ...
    if not accepted_category_keys:
        print("No acceptable bowl categories with annotations found. Stopping without merge.")
        raise SystemExit(1)
    ...
    for category_key, reason in zero_instance_valid_reasons.items():
        print(f"- {category_key} -> valid bowl label, zero instances copied ({reason})")
    print()
    ...
    print("Updated SOURCING_NOTES.md bowl row:")
    print(notes_row)
    print()
