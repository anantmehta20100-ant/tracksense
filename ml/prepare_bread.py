...
# Bread and clearly bread-family loaf products. Baguette / ciabatta / Maltese
# bread are all breads for the MVP's food class. Cake, pastry, sandwich,
# croissant, donut and similar are deliberately NOT here -- they get excluded
# and flagged for manual review rather than silently folded into bread.
VALID_BREAD_NAMES = {
    "bread",
    "breads",
    "loaf",
...
DATASETS = [
    {
        "id": "breadV3i",
        "name": "Bread.v3i",
        "zip_name": "Bread.v3i.coco.zip",
        "extract_dir": "dataset_bread_v3i",
    },
    {
        "id": "breadV1",
        "name": "BREAD.v1-bread-images",
        "zip_name": "BREAD.v1-bread-images.coco.zip",
        "extract_dir": "dataset_bread_v1",
    },
]
...
def require_zip_files() -> list[tuple[dict, Path]]:
    ...
    if missing:
        ...
        for path in missing:
            print(f"- {path}")
        print("Download both bread COCO zips to C:\\Users\\Anant\\Downloads\\ and rerun.")
        raise SystemExit(1)
...
def classify_category(name: str) -> tuple[bool, str]:
    ...
    if normalized in VALID_BREAD_NAMES:
        return True, f"{name!r} is a clearly valid bread/loaf category."

    if raw in AMBIGUOUS_OR_NONBREAD_NAMES or normalized in AMBIGUOUS_OR_NONBREAD_NAMES:
        return False, f"{name!r} is ambiguous or a non-bread baked good (cake/pastry/sandwich/etc.); excluded pending review."

    return False, f"{name!r} is not clearly a bread label; excluded pending review."
        ...
        for leaf in ("images", "labels"):
            ...
            for path in folder.iterdir():
                if path.is_file() and path.name != ".gitkeep":
                    path.unlink()
                    removed += 1
    print(f"Cleared {removed} existing bread image/label file(s), preserving .gitkeep placeholders.")
    print()
...
def summarize_records(
    ...
    for record in records:
        ...
        for category in categories:
            ...
            if accepted and instance_count > 0:
                ...
                decision = "valid bread label, but zero instances -> no labels copied (Roboflow root/supercategory placeholder)"
    ...
    if excluded_reasons:
        print("WARNING: Ambiguous or non-bread classes were found and will be excluded:")
...
def report_source_filename_duplicates(image_records: list[ImageRecord]) -> list[str]:
    ...
    print("=" * 31)
    if exact_duplicates:
        print(f"Exact duplicate source filenames detected across sources/splits: {len(exact_duplicates)} group(s) (showing up to 10).")
...
def unique_destination(split: str, source_image: Path, dataset_id: str, collision_reports: list[str]) -> Path:
    ...
    # Namespace every bread file by its source dataset id so the two bread
    # sources can never overwrite each other even on identical basenames.
    candidate = destination_dir / f"{dataset_id}_{source_image.name}"
    ...
    for index in range(1, 100000):
        ...
        if not renamed.exists():
            collision_reports.append(f"{split}: {dataset_id}_{source_image.name} -> {renamed.name}")
            return renamed
...
def update_sourcing_notes(
    ...
    source = "Bread.v3i.coco.zip; BREAD.v1-bread-images.coco.zip"
    if excluded_reasons:
        excluded = "; ".join(excluded_reasons)
        notes = (
            f"Combined {final_total_images} accepted images / {final_total_instances} annotations from both bread datasets; "
            f"excluded or ambiguous classes need manual review: {excluded}."
        )
    else:
        zero_note = ""
        if zero_instance_valid_reasons:
            zero_note = " Zero-instance Roboflow root/supercategory placeholders excluded."
        notes = (
            f"Combined {final_total_images} images / {final_total_instances} annotations from both bread datasets; "
            f"Baguette/Ciabatta/Maltese-Bread/Bread categories all remapped to bread global class id {DEST_CLASS_ID}. "
            f"Files namespaced by source id to prevent cross-source overwrites.{zero_note}"
        )
    new_row = f"| bread | {status} | {source} | {notes} |"

    lines = SOURCING_NOTES.read_text(encoding="utf-8").splitlines()
    updated: list[str] = []
    replaced = False
    for line in lines:
        if line.startswith("| bread |"):
...
def print_merge_summary(summary: dict[str, dict[str, int]], final_counts: dict[str, dict[str, int]]) -> None:
    ...
    print(f"TOTAL copied across both source datasets: images={total_images}, labels={total_labels}, instances={total_instances}")
    print()
...
def print_collision_summary(collision_reports: list[str]) -> list[str]:
    ...
    if not collision_reports:
        print("No destination filename collisions detected; source-id namespacing kept all files distinct.")
        print()
        return []
...
def main() -> None:
    ...
    accepted_category_keys, accepted_reasons, zero_instance_valid_reasons, excluded_reasons = summarize_records(all_records)
    if not accepted_category_keys:
        print("No acceptable bread categories with annotations found. Stopping without merge.")
        raise SystemExit(1)
    ...
    for category_key, reason in zero_instance_valid_reasons.items():
        print(f"- {category_key} -> valid bread label, zero instances copied ({reason})")
    print()
    ...
    print("Updated SOURCING_NOTES.md bread row:")
    print(notes_row)
    print()
