...
ZIP_NAME = "Hand detection.v2i.coco.zip"
DATASET_NAME = "Hand detection"
DATASET_URL = "https://universe.roboflow.com/hand-detection-k8b7v/hand-detection-c6sv3"
DATASET_ID = "handDetection"
DEST_CLASS = "hand"
...
def classify_category(name: str) -> tuple[bool, str]:
    ...
    if normalized in VALID_HAND_NAMES:
        return True, f"{name!r} is a clearly valid human-hand category."

    if raw in AMBIGUOUS_HAND_NAMES or normalized in AMBIGUOUS_HAND_NAMES:
        return False, f"{name!r} is ambiguous or not specific enough for the hand class."

    return False, f"{name!r} is not clearly a human-hand annotation label."
        ...
        for leaf in ("images", "labels"):
            ...
            for path in folder.iterdir():
                if path.is_file() and path.name != ".gitkeep":
                    path.unlink()
                    removed += 1
    print(f"Cleared {removed} existing hand image/label file(s), preserving .gitkeep placeholders.")
    print()
...
def summarize_records(
    ...
    for record in records:
        ...
        for category in categories:
            ...
            if accepted and instance_count > 0:
                ...
                decision = f"valid hand label, but zero instances -> no labels copied"
            else:
                decision = "excluded -> needs manual review"
    ...
    if excluded_reasons:
        print("WARNING: Ambiguous or non-hand classes were found and will be excluded:")
...
def update_sourcing_notes(
    ...
    if excluded_reasons:
        ...
        if zero_instance_valid_reasons:
            zero_labels = "; ".join(zero_instance_valid_reasons)
            zero_note = f" Zero-instance valid hand category present: {zero_labels}."
        notes = (
            f"Combined {final_total_images} images / {final_total_instances} annotations; "
            f"source hand annotations remapped to global class id {DEST_CLASS_ID}.{zero_note}"
        )
    new_row = f"| hand | {status} | {source} | {notes} |"
...
def main() -> None:
    ...
    if not accepted_category_keys:
        print("No acceptable hand categories with annotations found. Stopping without merge.")
        raise SystemExit(1)
    ...
    for category_key, reason in zero_instance_valid_reasons.items():
        print(f"- {category_key} -> valid hand label, zero instances copied ({reason})")
    print()
    ...
    print("Updated SOURCING_NOTES.md hand row:")
    print(notes_row)
    print()
