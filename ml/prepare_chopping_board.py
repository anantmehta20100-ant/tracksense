...
ZIP_NAME = "Cutting board.v1i.coco.zip"
DATASET_NAME = "Cutting board"
DATASET_URL = None
DATASET_ID = "cuttingBoardV1i"
DEST_CLASS = "chopping_board"
...
# "cutting board" and "chopping board" are the same object; both accepted.
VALID_BOARD_NAMES = {
    "cuttingboard",
    "cuttingboards",
    "choppingboard",
...
def classify_category(name: str) -> tuple[bool, str]:
    ...
    if normalized in VALID_BOARD_NAMES:
        return True, f"{name!r} is a clearly valid cutting/chopping board category."

    if raw in AMBIGUOUS_BOARD_NAMES or normalized in AMBIGUOUS_BOARD_NAMES:
        return False, f"{name!r} is ambiguous or a different surface type (counter/table/tray), not chopping_board."

    return False, f"{name!r} is not clearly a chopping board annotation label."
        ...
        for leaf in ("images", "labels"):
            ...
            for path in folder.iterdir():
                if path.is_file() and path.name != ".gitkeep":
                    path.unlink()
                    removed += 1
    print(f"Cleared {removed} existing chopping_board image/label file(s), preserving .gitkeep placeholders.")
    print()
...
def summarize_records(
    ...
    for record in records:
        ...
        for category in categories:
            ...
            if accepted and instance_count > 0:
                ...
                decision = "valid board label, but zero instances -> no labels copied (Roboflow root/supercategory placeholder)"
    ...
    if excluded_reasons:
        print("WARNING: Ambiguous or non-board classes were found and will be excluded:")
...
def update_sourcing_notes(
    ...
    if excluded_reasons:
        ...
        notes = (
            ...
            f"(no source valid/test split -- auto 80/10/10 split applied); "
            f"'Cutting-board' remapped to chopping_board global class id {DEST_CLASS_ID}.{zero_note}"
        )
    new_row = f"| chopping_board | {status} | {source} | {notes} |"

    lines = SOURCING_NOTES.read_text(encoding="utf-8").splitlines()
    updated: list[str] = []
    replaced = False
    for line in lines:
        if line.startswith("| chopping_board |"):
...
def main() -> None:
    ...
    if not accepted_category_keys:
        print("No acceptable chopping_board categories with annotations found. Stopping without merge.")
        raise SystemExit(1)
    ...
    for category_key, reason in zero_instance_valid_reasons.items():
        print(f"- {category_key} -> valid board label, zero instances copied ({reason})")
    print()
    ...
    print("Updated SOURCING_NOTES.md chopping_board row:")
    print(notes_row)
    print()
