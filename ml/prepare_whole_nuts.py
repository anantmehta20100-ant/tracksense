...
ZIP_NAME = "Nut classification.v1i.coco.zip"
DATASET_NAME = "Nut classification"
DATASET_URL = "https://universe.roboflow.com/dataset-r3opz/nut-classification-jmkxg"
DATASET_ID = "wholeNuts"
DEST_CLASS = "whole_nuts"
...
def classify_category(name: str) -> tuple[bool, str]:
    ...
    if normalized in VALID_WHOLE_NUT_NAMES:
        return True, f"{name!r} is a clearly valid whole-nut category."
    return False, f"{name!r} is not clearly a whole-nut category."
        ...
        for leaf in ("images", "labels"):
            ...
            for path in folder.iterdir():
                if path.is_file() and path.name != ".gitkeep":
                    path.unlink()
                    removed += 1
    print(f"Cleared {removed} existing whole_nuts image/label file(s), preserving .gitkeep placeholders.")
    print()
...
def summarize_records(records: list[JsonRecord]) -> tuple[set[tuple[str, int]], dict[str, str], dict[str, str]]:
    ...
    for record in records:
        ...
        for category in categories:
            ...
            decision = f"accepted -> {DEST_CLASS}" if accepted else "excluded -> needs manual review"
    ...
    if excluded_reasons:
        print("WARNING: Ambiguous or non-nut classes were found and will be excluded:")
...
def update_sourcing_notes(
    ...
    if excluded_reasons:
        ...
        notes = (
            ...
            f"all inspected nut categories remapped to whole_nuts global class id {DEST_CLASS_ID}."
        )
    new_row = f"| whole_nuts | {status} | {source} | {notes} |"

    lines = SOURCING_NOTES.read_text(encoding="utf-8").splitlines()
    updated: list[str] = []
    replaced = False
    for line in lines:
        if line.startswith("| whole_nuts |"):
...
def main() -> None:
    ...
    accepted_category_keys, accepted_reasons, excluded_reasons = summarize_records(records)
    if not accepted_category_keys:
        print("No acceptable whole_nuts categories found. Stopping without merge.")
        raise SystemExit(1)
    ...
    print("Updated SOURCING_NOTES.md whole_nuts row:")
    print(notes_row)
    print()
